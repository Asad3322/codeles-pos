      ...
      <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              <div className="flex items-center justify-between">
                <p className="text-sm text-zinc-500 dark:text-zinc-400">Total bakery items</p>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Adjust bakery item quantity manually when stock changes or discrepancies occur.
              </p>
              <Button
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex items-center gap-2 text-sm text-zinc-500 dark:text-zinc-400">
                ...
                <span>Total active items: {inventory.filter((item) => item.isActive).length}</span>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <CardTitle>Finished Stock List</CardTitle>
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Current inventory levels and bakery item details.
              </p>
            </div>
            <Button className="bg-emerald-600 hover:bg-emerald-700 text-white font-medium" onClick={() => openAdjust(selectedItem ?? inventory[0] ?? { _id: "", name: "", sku: "", stock: 0, sellingPrice: 0, isActive: true })} disabled={inventory.length === 0}>
          ...
          <CardContent className="overflow-x-auto">
            {isLoading ? (
              <p className="py-8 text-center text-sm text-zinc-500 dark:text-zinc-400">Loading stock...</p>
            ) : (
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b text-left text-zinc-500 dark:text-zinc-400">
                    <th className="pb-3 pr-4 font-medium">Bakery Item</th>
                  ...
                  {inventory.map((item) => (
                    <tr key={item._id} className="border-b border-zinc-100 dark:border-zinc-800">
                      ...
                      <td className="py-3 pr-4">
                        <span className={item.stock <= 5 ? "text-amber-600 font-semibold" : "text-zinc-900 dark:text-zinc-50"}>
                          {item.stock}
                        </span>
                      </td>
                      <td className="py-3 pr-4">
            ...
            {!isLoading && inventory.length === 0 && (
              <p className="py-8 text-center text-zinc-500 dark:text-zinc-400">No stock data available.</p>
            )}
          </CardContent>
        </Card>
      </div>
